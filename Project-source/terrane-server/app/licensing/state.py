...
The activation credential is written by the admin backend to `licenses/active.forge`
(JSON envelope: `{"method": "offline"|"online", "credential": "<blob or short code>"}`;
a bare offline blob is also accepted). This service only reads the file: verify once at
startup + re-verify every TERRANE_LICENSE_RECHECK_SECONDS (expiry / CRL revocation /
clock rollback / online lease renewal); any exception transitions to the locked state.
"""
...
_BYPASS = Verdict("active", "license_not_required")  # synthetic unlocked verdict when gating is off (open-source edition)
...
class LicenseState:
    """Single-instance License state machine. Reading the verdict is atomic (attribute replacement); writes happen only within this class."""

    def __init__(self, settings: Settings) -> None:
        self._settings = settings
        # install_id and the activation envelope share the licenses/ shared volume: the three
        # components share one install_id = one deployment identity, stable across container
        # restarts / DB migrations (design 02 §anti-clone).
        ...
        self._verify_lock = threading.Lock()  # serialize verification: no concurrent re-entry hitting the edge (avoid rate limiting)
        self.initial_checked = False  # readyz readiness signal: the first verification has completed

    @property
    def required(self) -> bool:
        """Whether gating is enabled (open-source edition defaults to False -> pass through entirely)."""
    ...
    def _load_crl(self) -> tuple[set[str], int | None, str | None]:
        """Read and verify the CRL file; untrusted/missing -> empty set (the License's own validity period is still the fallback)."""
        ...
        except Exception:  # noqa: BLE001 -- a CRL error is non-fatal; the License validity period is still the fallback
            pass
        return revoked, None, None

    def verify_now(self) -> Verdict:
        """Synchronously run one full verification and update state; any exception -> locked (fail-closed). The serial lock prevents concurrent re-entry hitting the edge."""
    ...
    def _verify_now_locked(self) -> Verdict:
        ...
        if envelope is None:
            ...
            except Exception:  # noqa: BLE001 -- do not leak internal verification details (zero-leakage requirement)
    ...
    def verify_if_stale(self, max_age_seconds: float) -> None:
        """Re-verify on demand but throttled: only really verify if more than max_age_seconds has
        elapsed since the last verification, otherwise serve the cache. Used by the status endpoint
        so revocation/deletion is reflected near-instantly even in the active state, while limiting the frequency of hitting the edge in online mode."""
        if not self._settings.license_required:
            return  # gating off: do not verify
        with self._verify_lock:
    ...
    def try_credential(self, method: str, credential: str) -> Verdict:
        """Verify a candidate credential (without persisting it or changing the current verdict) -- the pre-check for activation."""
        try:
            return self._verify(method, credential)
        except Exception:  # noqa: BLE001 -- fail-closed, do not leak details
    ...
    def _verify(self, method: str, credential: str) -> Verdict:
        if method == METHOD_ONLINE:
            ...
            # Same online code and already holding a lease -> renew (offline is allowed within the
            # signature grace period); a new code (e.g. reissued after the old ticket was revoked)
            # -> must re-activate, never renew the old ticket with the old token.
    ...
    async def start(self) -> None:
        if not self._settings.license_required:
            ...
            log.info("license.disabled")  # open-source gating off: no verification, no recheck loop started
            return
