...
def validate_password_policy(
    ...
    *,
    ...
    `forbid_identity` is tunable: Terrane's factory default password equals the email
    and must be usable, so it is configured as False by default.
    """
...
def new_token(nbytes: int = 32) -> str:
    """URL-safe one-time token (>= 32 bytes of entropy), used for password reset / API keys."""
