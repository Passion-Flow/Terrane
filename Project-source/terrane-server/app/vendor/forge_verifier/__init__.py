...
class ForgeVerifier:
    def __init__(self, edge_url: str | None = None, *, install_id_path: str | None = None) -> None:
        ...
        # Anti-clone identity (design 07): injected UID (container/cluster authority), a multi-signal
        # vector, and a random install_id from the first activation.
        # Fallback: with no injected UID, use the deployment fingerprint as deployment_uid -- multiple
        # components on the same host (server/admin/gateway) share one fingerprint -> edge dedupes by
        # UID to "one seat per deployment", eliminating the binding_mismatch flapping where components
        # fight over a seat. The fingerprint comes from hardware and cannot be injected, so anti-clone /
        # anti-forgery is not weakened (a cloned machine has a different fingerprint -> different UID -> still rejected).
