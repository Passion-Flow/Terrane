"""Xinchuang (domestic) DB adaptation matrix -- DATABASE_TYPE -> SQLAlchemy dialect + async driver mapping.

The control plane's ORM/migrations are developed on postgres but must be switchable
to domestic databases. This module is the **only** resolution point for
type -> (dialect+driver+connect_args); session.py / platform.py both reuse it and
do not change the default postgres behavior (DATABASE_TYPE unset = postgres ->
postgresql+asyncpg, identical to the original).

Compatibility groups (see docs/xinchuang-db-matrix.md):
  - PG protocol family (reuse postgresql+asyncpg directly): postgres / opengauss / polardb-pg /
    gaussdb / kingbase. opengauss/gaussdb/kingbase/polardb-pg are highly wire-compatible with
    PostgreSQL and can connect via asyncpg directly; a few DBs require enabling PG compatibility
    mode / setting the password encryption algorithm on the server (see "Known limitations" in the docs).
  ...
  - Dameng DM8 (dameng): third-party SQLAlchemy dialect sqlalchemy-dm, no official async driver ->
    degrades to a synchronous dialect URL (requires a greenlet bridge; under this control plane's
    async stack Dameng is classified as "experimental/limited").
...
xinchuang-dameng / xinchuang-oracle). When a driver is missing, raise a clear ImportError instead of crashing the default path.
"""
...
# Default (fallback) database type -- never changed.
DEFAULT_DATABASE_TYPE = "postgres"


@dataclass(frozen=True)
class DialectSpec:
    """The resolution result for a single DATABASE_TYPE."""

    db_type: str
    drivername: str  # the "dialect+driver" of the SQLAlchemy URL
    driver_module: str  # import name used in the error hint when missing
    extra: str  # name of the pyproject extra that installs this driver
...
_MATRIX: dict[str, DialectSpec] = {
    # -- PG protocol family: reuse postgresql+asyncpg directly --
    ...
    "opengauss": DialectSpec(
        ...
        "openGauss is PG-protocol-compatible, connect via asyncpg; the server needs "
        "password_encryption_type=1 (sha256) or compatibility mode enabled, otherwise the asyncpg SCRAM handshake may fail.",
    ),
    "polardb-pg": DialectSpec(
        "polardb-pg", "postgresql+asyncpg", "asyncpg", "xinchuang-pg", True,
        "Aliyun PolarDB for PostgreSQL, fully PG-protocol-compatible, asyncpg direct connection.",
    ),
    "gaussdb": DialectSpec(
    ...
    "oceanbase": DialectSpec(
        ...
        "OceanBase (MySQL mode), connect over the MySQL protocol; an ob mysql tenant is recommended.",
    ),
    "tidb": DialectSpec(
        "tidb", "mysql+asyncmy", "asyncmy", "xinchuang-mysql", True,
        "TiDB is MySQL-protocol-compatible, asyncmy direct connection.",
    ),
    "polardb-x": DialectSpec(
        "polardb-x", "mysql+asyncmy", "asyncmy", "xinchuang-mysql", True,
        "Aliyun PolarDB-X (distributed MySQL), connect over the MySQL protocol.",
    ),
    # -- Dameng DM8: third-party sqlalchemy-dm dialect, no official async driver -> sync bridge, limited --
    "dameng": DialectSpec(
        ...
        "bridge under the async stack with limited concurrency, classified as experimental/limited (see the docs).",
    ),
    "dm8": DialectSpec(
        "dameng", "dm+dmPython", "dmPython", "xinchuang-dameng", False,
        "Alias for dameng.",
...
def ensure_driver(spec: DialectSpec) -> None:
    """Confirm the corresponding async driver can be imported; on failure give clear guidance on which extra to install.

    Does not force the import at resolution time (so unit tests / CLIs that only build
    a URL need not install all drivers); only called right before actually creating the engine.
    """
...
def connect_args_for(spec: DialectSpec, *, ssl_mode: str) -> dict:
    ...
    - asyncpg (PG protocol family): ssl takes a bool/SSLContext, require/verify-* -> on.
    - asyncmy (MySQL protocol family): defaults to utf8mb4; ssl via the ssl dict (require -> {} = on).
