...
A session is an opaque server-side record indexed by a random sid; the cookie carries only the
sid. Sliding renewal up to the absolute TTL. A per-user index set supports global logout (revoke
all sessions on password change/reset/role change/disable).
key prefix: terrane_admin_session:
"""
...
class SessionService:
    ...
    def _key(self, sid: str) -> str:
        return f"terrane_admin_session:{sid}"

    def _user_index(self, user_id: str) -> str:
        return f"terrane_admin_session:user:{user_id}"

    async def create(self, *, user_id: str, role: str, ip: str, ua: str,
                     twofa_verified: bool, permissions: list[str], name: str | None = None,
                     absolute_ttl_seconds: int | None = None) -> str:
    ...
    async def get(self, sid: str) -> dict[str, Any] | None:
        ...
        if now >= data["absolute_expiry"]:
            ...
            return None
        # Sliding renewal (bounded by the absolute expiry cap)
